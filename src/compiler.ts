...
<<<<<<< HEAD
import { compileMDV, generateGlobalComponentsModule } from "./parser";
...
<<<<<<< HEAD
    ...
    async function watchAll(dir: string = srcRoot, viteServer?: any) {
        ...
        const watcher = chokidar.watch("**/*.v.md", {
            cwd: dir,
            persistent: true,
            ignoreInitial: false,
        })
    ...
    function writeGlobalComponentsDTS(dir: string = srcRoot) {
        ...
        function scanDir(d: string) {
            ...
            for (const e of entries) {
                ...
                else if (e.isFile() && e.name.endsWith('.v.md')) mdvFiles.push(fullPath)
            }
        }
