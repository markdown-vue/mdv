...
export function mdvPlugin(options = {}) {
    const { compileMDVFile, compileAllMDVFiles, writeGlobalComponentsDTS, extension, cacheDir, srcRoot, srcName, compiledTimestamps, } = Compiler(options);
    ...
    return {
        ...
        async configureServer(server) {
            ...
            if (files.length === 0)
                console.warn("\x1b[33m%s\x1b[36m%s\x1b[33m%s\x1b[36m%s\x1b[33m%s\x1b[0m", `🚨🚨🚨 No`, ` .v.md `, `files found in`, ` /${srcName} `, `directory. Make sure you have the right extension.`);
