 ...
 */
export async function markdownToAST(mdContent, shikiPath, customComponents = {}) {
    ...
    while (i < tokens.length) {
        ...
        // Handle closing tags
            ...
            if (!components.includes(finalTag))
                components.push(finalTag);
            if (stack.length > 0)
                stack[stack.length - 1].children.push(u("html", html));
            else
                astChildren.push(u("html", html));
        }
        // Inline content
            ...
            if (stack.length > 0)
                stack[stack.length - 1].children.push(u("html", inlineContent));
            else
                astChildren.push(u("html", inlineContent));
        }
        // Fenced code blocks
            ...
            if (stack.length > 0)
                stack[stack.length - 1].children.push(u("html", codeHtml));
            else
                astChildren.push(u("html", codeHtml));
        }
        i++;
    }
    return {
        ast: u("root", astChildren),
        shikis,
        imports,
        components,
    };
...
export function transformAST(ast) {
    ...
    visit(ast, (node) => {
        if (node.type === "html" && node.value) {
            ...
            value = value.replace(/:\[([^\]]+)\](?:\{\s*(?:::(\w+))?([\s\S]*?)\})?/g, (_, expr, comp, props) => {
                const tag = comp || "span";
            ...
            value = value.replace(/\[([^\]]+)\](?:\{\s*(?:::(\w+))?([\s\S]*?)\})?/g, (_, text, comp, props) => {
                const tag = comp || "span";
                const propStr = props ? props.trim() : "";
...
<table>
  <thead>
    <tr>${node.headers?.map((h) => `<th>${h}</th>`).join("")}</tr>
  </thead>
  <tbody>
 ...
 */
export function astToTemplate(ast) {
    let template = "";
    visit(ast, (node) => {
 ...
 */
export function extractScriptStyle(mdContent) {
...
export async function compileMDV(mdContent, metaPath, shikiPath, options = {}) {
 ...
 */
export function generateGlobalComponentsModule(paths) {
    const imports = paths
        .map((p) => {
...
function getComponentName(rawPath) {
 ...
 * @returns
 */
function stripCodeBlocks(mdContent) {
